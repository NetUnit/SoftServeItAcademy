...
urlpatterns = [
    # *** just to test Django API view *** #
    re_path(r'^accounts/list/$', accounts_list_view, name='accounts_list_view'),

    # *** REGISTER FBV + CBV *** #
    re_path(r'^accounts/register-fbv/$', register_user_view, name='register_fbv'),
    re_path(r'^accounts/register-cbv/$', RegisterView.as_view(), name='register_cbv'),

    # *** LOGIN FBV + CBV + Fail *** #
    re_path(r'^accounts/login-cbv/$', CustomLoginView.as_view(), name='login_cbv'),
    re_path(r'^accounts/login-fbv/$', login_user_view, name='login_fbv'),
