...
urlpatterns = [
    ...
    # *** Create View *** #
    re_path(
        r'^order/create/(?P<product_id>\d+)/$',
        order_create_view,
        name='order_create_view'),

    # *** Delete View *** #
    re_path(
        r'^order/delete/(?P<order_id>\d+)/$',
        order_remove_view,
        name='order_remove_view'),
    ...
    re_path(
        r'^api/order/payment/$',
        api_process_payment_view,
        name='api_process_payment_view'),
    re_path(
        r'^order/download/(?P<order_id>\d+)$',
        order_download_view,
        name='order_download_view'),
    re_path(
        r'^order/payment-done/$',
        TemplateView.as_view(template_name="orders/payment_done.html"),
        name='payment_done'),
    re_path(
        r'^order/payment-canceled/$',
        TemplateView.as_view(template_name="orders/payment_canceled.html"),
        name='payment_canceled'),

    # *** Order Pay View set 2 *** #
